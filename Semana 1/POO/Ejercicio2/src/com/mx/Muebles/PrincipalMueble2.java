...
//	Mueble miMueble = new Mueble("Verde", 5000.00, 1, "tela", "sala");
	...
	public static void main(String[] args) {
		...
		}while(opcionMenuPrincipal != (listaMenuPrincipal.size() - 1));
	}
	...
	private static void bloqueSwitchMenuPrincipal(int opcion) {
			...
			try {
				...
				if(indiceMueble >= 0 && indiceMueble <mueblesImp.getNumElementos()) {
					...
					do {
						...
						System.out.print("Elige una opción: ");
						try {
							opcionMenuEditar = datos.nextInt();
							datos.nextLine();
							bloqueSwitchEditar(opcionMenuEditar);
						}catch(InputMismatchException e){
							System.err.println(e+" Ingresa un tipo de dato entero!!");
							datos.nextLine();
						}
					}while(opcionMenuEditar != (listaMenuEditar.size()-1));
					//Guardando los cambios
					muebleAux.setColor(color);
					muebleAux.setMaterial(material);
					muebleAux.setNoPiezas(noPiezas);
					muebleAux.setPrecio(precio);
					muebleAux.setUbicacion(ubicacion);
					mueblesImp.update(indiceMueble, muebleAux);
		...
		case 4:
			System.out.println(opcion+".- "+listaMenuPrincipal.get(opcion));
			muebleAux = buscar();
			System.out.println(muebleAux == null ? "No se encontro el mueble":
				"Los datos del mueble son: "+muebleAux);
			break;
		//Eliminar
		case 5:
			System.out.println(opcion+".- "+listaMenuPrincipal.get(opcion));
	...
	private static Mueble buscar() {
		...
		try {
			...
			datos.nextLine();
			if(indiceMueble >= 0 && indiceMueble <mueblesImp.getNumElementos()) {
				aux = mueblesImp.buscar(indiceMueble);
			}
		}catch(InputMismatchException e){
			System.err.println(e+" Ingresa un tipo de dato entero!!");
			datos.nextLine();
	...
	private static void bloqueSwitchEditar(int opcion) {
		switch(opcion) {
		case ELEMENTO_COLOR:
			System.out.println(opcion+".- "+listaMenuPrincipal.get(opcion));
			leeElementoMueble("Ingresa el nuevo color", ELEMENTO_COLOR);
			break;
		case ELEMENTO_PRECIO:
			System.out.println(opcion+".- "+listaMenuPrincipal.get(opcion));
			leeElementoMueble("Ingresa el nuevo precio", ELEMENTO_PRECIO);
			break;
		case ELEMENTO_NO_PIEZAS:
			System.out.println(opcion+".- "+listaMenuPrincipal.get(opcion));
			leeElementoMueble("Ingresa el nuevo numero de piezas", ELEMENTO_NO_PIEZAS);
			break;
		case ELEMENTO_MATERIAL:
			System.out.println(opcion+".- "+listaMenuPrincipal.get(opcion));
			leeElementoMueble("Ingresa el nuevo material", ELEMENTO_MATERIAL);
			break;
		case ELEMENTO_UBICACION:
			System.out.println(opcion+".- "+listaMenuPrincipal.get(opcion));
			leeElementoMueble("Ingresa el nuevo color ubicacion", ELEMENTO_MATERIAL);
			break;
		case 6:
