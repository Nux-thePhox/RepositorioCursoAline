...
public class Principal2 {
	...
	private static void bloqueSwitchMenuPrincipal(int opcion) {
		...
		case 0:
			...
			boolean indiceEncontrado;
			do {
				leeElementoOpening("Ingresa el nombre del opening", ELEMENTO_NOMBRE);
				indiceABuscar = buscaOpeningPorNombre(nombre);
				indiceEncontrado = indiceABuscar != OPENING_NO_ENCONTRADO;
				if(indiceEncontrado) {
					System.out.println("Ese nombre de opening ya esta registrado, ingresa otro");
				}
			...
			leeElementoOpening("Ingresa los segundos de duracion del opening", ELEMENTO_SEGUNDOS_DURACION);
			Opening nuevoOp 
		...
		case 2:
			...
			int indiceOpening = buscaOpeningPorNombre(nombre);
		...
		case 3:
			...
			//Se lee el nombre a buscar y se guarda en la variable global nombre
			leeElementoOpening("Ingresa el nombre del opening a buscar", ELEMENTO_NOMBRE);
			int indiceOpeningBuscado = buscaOpeningPorNombre(nombre);
		...
		case 4:
			...
			//Se lee el nombre a buscar y se guarda en la variable global nombre
			leeElementoOpening("Ingresa el nombre del opening a buscar", ELEMENTO_NOMBRE);
			int indiceOpeningEliminar = buscaOpeningPorNombre(nombre);
			if(indiceOpeningEliminar == OPENING_NO_ENCONTRADO) {
				System.out.println("El opening no se encuentra en la lista almacenada");
			}else {
				listaOpenings.remove(indiceOpeningEliminar);
				System.out.println("Se ha eliminado el opening de la lista");
			}
			break;
		case 5:
			System.out.println(opcion+".- Contando los elementos de la lista");
			System.out.println("Existen "+listaOpenings.size()+" elementos de la lista");
			break;
		case 6:
			System.out.println(opcion+".- Vaciando la lista");
			do {
				datoInvalido = true;
				System.out.print("Estas seguro de eliminar la lista? (Oprime 0 para SI, 1 para NO): ");
				try {
					opcionVaciar = scanner.nextInt();
					scanner.nextLine();
					datoInvalido = false;
					if(opcionVaciar == 0) {
						...
						System.out.println("uff eso estuvo cerca.");
					}
				}catch(InputMismatchException e) {
					scanner.nextLine();
					System.err.println(e+" Ingresa un tipo de dato entero!!");
	...
	private static int buscaOpeningPorNombre(String nombreABuscar) {
		int i=0;
		for(Opening openingARevisar: listaOpenings) {
			if(openingARevisar.getNombre().equals(nombreABuscar)) {
				System.out.println("El opening: "+nombreABuscar+" fue encontrado!");
				return i;
			}
			i++;
		}
